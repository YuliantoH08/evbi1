    ...
    <div class="col-12">
        <div class="card card-outline card-primary shadow rounded-0">
            <div class="card-body rounded-0">
                <h2>Event List<?= isset($department['name']) ? $department['name'] : "" ?> </h2>
                ...
                $editors = $conn->query("SELECT * FROM `editor_list` where id in (SELECT editor_id FROM archive_list where `status` = 1 and curriculum_id in (SELECT id from curriculum_list {$wheredid} ))");
